...
def deploy_web_apps():
    ...
    try:
        run("sudo /usr/bin/perl -p -i -e 's/" + env.run_service_port + "/" + env.stop_service_port + "/g' /etc/httpd_gbs/conf.d/gbs_spring.conf")
        run('sudo systemctl reload httpd_gbs')
        run("sudo /usr/bin/perl -p -i -e 's/" + env.run_switch + "/" + env.stop_switch + "/g' "  + env.deploy_dir + switch_filename)        

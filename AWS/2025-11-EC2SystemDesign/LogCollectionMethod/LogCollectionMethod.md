...
- [3. CloudWatch Logs Agent インストール](#3-cloudwatch-logs-agent-インストール)
  - [3.0 前提条件: Amazon Linux 2023でのrsyslog有効化](#30-前提条件-amazon-linux-2023でのrsyslog有効化)
...
graph TB
    ...
    CHK -->|No| DL[Download Agent<br/>from S3/Internet]
    CHK -->|Yes| CONF
    
    DL --> INST[Install Package<br/>rpm/deb]
    INST --> IAMCHK{IAM Role attached?}
    ...
    SSMCHK -->|No| LOCAL[Use local config]
    
    SSMPUSH --> START_AGENT[Start Agent]
    LOCAL --> START_AGENT
    
    START_AGENT --> VERIFY[Verify Logs in<br/>CloudWatch]
    VERIFY --> END[Complete]
...
# httpdログディレクトリの権限を調整（cwagentが読めるように）
sudo chmod 755 /var/log/httpd の必要があります。
